...
const dataSuggestions = computed(() => props.modelValue !== '' ? props.options.filter(place => place.toLowerCase().includes(props.modelValue.toLowerCase())) : props.options )
...
<template>
    <div class="inline-flex flex-col justify-center relative text-gray-500 self-start w-[80%] pt-[2px]">
        <div class="relative">
            <input
            type="text"
            class="border-none p-2 pl-1 border-gray-200 bg-white w-full focus:outline-0" placeholder="Where to?"
            @focus="focusInput"
            @blur="blurInput"
            :value="modelValue"
